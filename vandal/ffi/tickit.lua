...
okay, lib = pcall(ffi.load, "libtickit.so.0")

if not okay then
    okay, lib = pcall(ffi.load, "tickit")
end

if not okay then
    error("Couldn't find a suitable tickit library to load.")
end

local tickit = { DECLS = decls, C = lib }
tickit.L = tickit

--  TickitRect

function tickit.tickit_rect_bottom(rect) return rect.top + rect.lines end
function tickit.tickit_rect_right( rect) return rect.left + rect.cols end

--  Some convenient mutators for TickitStringPos structs

function tickit.tickit_stringpos_zero(pos)
    pos.bytes = 0
    pos.codepoints = 0
    pos.graphemes = 0
    pos.columns = 0
...
tickit.INIT_TICKIT_STRINGPOS_LIMIT_NONE = ffi.new("TickitStringPos", -1, -1, -1, -1)

function tickit.tickit_stringpos_limit_none(pos)
    pos.bytes = -1
    pos.codepoints = -1
    pos.graphemes = -1
    pos.columns = -1
...
function tickit.INIT_TICKIT_STRINGPOS_LIMIT_BYTES(v)
    return ffi.new("TickitStringPos", v, -1, -1, -1)
end

function tickit.tickit_stringpos_limit_bytes(pos, bytes)
    pos.bytes = bytes
    pos.codepoints = -1
    pos.graphemes = -1
    pos.columns = -1
...
function tickit.INIT_TICKIT_STRINGPOS_LIMIT_CODEPOINTS(v)
    return ffi.new("TickitStringPos", -1, v, -1, -1)
end

function tickit.tickit_stringpos_limit_codepoints(pos, codepoints)
    pos.bytes = -1
...
function tickit.INIT_TICKIT_STRINGPOS_LIMIT_GRAPHEMES(v)
    return ffi.new("TickitStringPos", -1, -1, v, -1)
end

function tickit.tickit_stringpos_limit_graphemes(pos, graphemes)
    pos.bytes = -1
    pos.codepoints = -1
...
function tickit.INIT_TICKIT_STRINGPOS_LIMIT_COLUMNS(v)
    return ffi.new("TickitStringPos", -1, -1, -1, v)
end

function tickit.tickit_stringpos_limit_columns(pos, columns)
    pos.bytes = -1
    pos.codepoints = -1
    pos.graphemes = -1
    pos.columns = columns
...
function tickit.tickit_window_top(win)
    return lib.tickit_window_get_geometry(win).top
end

function tickit.tickit_window_left(win)
    return lib.tickit_window_get_geometry(win).left
end

function tickit.tickit_window_lines(win)
    return lib.tickit_window_get_geometry(win).lines
end

function tickit.tickit_window_cols(win)
    return lib.tickit_window_get_geometry(win).cols
end

vandal.ffi.tickit = setmetatable({ }, {
    __index = function(self, key)
        return tickit[key] or lib[key]
    end,

    __newindex = function() error "Vandal error: Cannot modify the `vandal.ffi.tickit` table." end,
})

return vandal.ffi.tickit
