if not vandal.ffi then
    vandal.ffi = { }
elseif vandal.ffi.termkey then
    error "wut?"
end
...
local decls = [[
enum {
    TERMKEY_VERSION_MAJOR =  0,
    TERMKEY_VERSION_MINOR = 20,
};
...
okay, lib = pcall(ffi.load, "libtermkey.so.1")

if not okay then
    okay, lib = pcall(ffi.load, "termkey")
end

if not okay then
    error("Couldn't find a suitable termkey library to load.")
end

local termkey = { DECLS = decls }

termkey.TERMKEY_FORMAT_VIM = bit.bor(lib.TERMKEY_FORMAT_ALTISMETA, lib.TERMKEY_FORMAT_WRAPBRACKET)
termkey.TERMKEY_FORMAT_URWID = bit.bor(lib.TERMKEY_FORMAT_LONGMOD, lib.TERMKEY_FORMAT_ALTISMETA, lib.TERMKEY_FORMAT_LOWERMOD, lib.TERMKEY_FORMAT_SPACEMOD, lib.TERMKEY_FORMAT_LOWERSPACE)

function termkey.TERMKEY_CHECK_VERSION()
    return lib.termkey_check_version(ffi.C.TERMKEY_VERSION_MAJOR, ffi.C.TERMKEY_VERSION_MINOR)
end

vandal.ffi.termkey = setmetatable({ }, {
    __index = function(self, key)
        return termkey[key] or lib[key]
    end,

    __newindex = function() error "Vandal error: Cannot modify the `vandal.ffi.termkey` table." end,
})

return vandal.ffi.termkey
